...
// ── Keep Your Exact EmailData Type ──────────────────
...
// ── Core Send Function (replaces nodemailer) ─────────
async function sendEmail(
  ...
  try {
    ...
    console.log(`📧 Proxy URL: ${EMAIL_PROXY_URL}`)
...
export async function sendAdminNotificationEmail(
  ...
  try {
    ...
    const result = await sendEmail(
      adminEmail,
      `New Pickup Request - ${data.rcrcName || data.customerName}`,
      html
    )
...
export async function sendRequestorConfirmationEmail(
  data: EmailData
) {
  ...
  try {
      ...
      <div style="font-family: Arial, sans-serif; 
        ...
        <div style="border: 1px solid #e5e7eb; 
          ...
          <p style="color: #374151; font-size: 15px;">
            ...
            successfully submitted. Our team will contact 
            you shortly to confirm the pickup schedule.
          </p>
          <table style="width: 100%; 
                        border-collapse: collapse; 
                        font-size: 14px;">
    ...
    const result = await sendEmail(
      data.customerEmail,
      'Your Scrap Pickup Request Has Been Received',
      html
    )

    if (result.success) {
      console.log('✅ Requestor email sent to:', 
                  data.customerEmail)
    }
    return result
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; 
        ...
        <div style="border: 1px solid #e5e7eb; 
          ...
          <p style="color: #374151; font-size: 15px;">
            An admin has requested a password reset for 
            your account. Click the button below to set 
            a new password.
          </p>
