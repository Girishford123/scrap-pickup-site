...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // ✅ Keep your existing simple auth check
      if (
        email    === process.env.NEXT_PUBLIC_ADMIN_EMAIL    ||
        email    === 'admin@ford.com'
      ) {
        // You can keep hardcoded or move to env var
        ...
        // ✅ FIX: Save BOTH keys so both dashboard AND navbar work!
        // Key 1: for admin dashboard auth check
        localStorage.setItem('isAdminLoggedIn', 'true')
        localStorage.setItem('adminEmail', email)

        // Key 2: for Navbar to read user session and HIDE Admin Login tab
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white rounded-b-2xl shadow-xl p-8">
        ...
        <div className="mb-6">
          <Link
            href="/"
            className="inline-flex items-center text-sm text-gray-500 hover:text-[#003478] transition-colors"
          >
